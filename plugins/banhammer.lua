...
local function kick_ban_res(extra, success, result)
	  ...
	  if success == 0 then
		return send_large_msg(receiver, "لازم رد لو معرف حمبي🌚💔")
	  end
      ...
      local get_cmd = extra.get_cmd
       if get_cmd == "ادفر" then
         if member_id == from_id then
              send_large_msg(receiver, "لا تستطيع طرد نفسك غبي😼👏🏻")
			return
         end
         if is_momod2(member_id, chat_id) and not is_admin2(sender) then
         send_large_msg(receiver, "لا تستطيع طرد الشرفين😼👏🏻")
			return
         end
		 kick_user(member_id, chat_id)
      elseif get_cmd == 'حظر' then
        if is_momod2(member_id, chat_id) and not is_admin2(sender) then
        send_large_msg(receiver, "لا تستطيع حظر المشرفين😼👏🏻")
			return
        end
send_large_msg(receiver, '|⚜️ تم حظر العضو😽👏🏻\n|⚜️ معرفه » @'..member..' «\n|⚜️ ايديه » '..member_id..' «')
		ban_user(member_id, chat_id)
      elseif get_cmd == 'الغاء الحظر' then
send_large_msg(receiver, '|⚜️ تم الغاء حظره😼👏🏻\n|⚜️ معرفه » @'..member..' «\n|⚜️ ايديه » '..member_id..' «')
        local hash =  'banned:'..chat_id
        redis:srem(hash, member_id)
return '|⚜️ العضو'..user_id..'تم الغاء حظره'
      elseif get_cmd == 'حظر عام' then
send_large_msg(receiver, '|⚜️ تم حظره عام😼👏🏻\n|⚜️ معرفه » @'..member..' «\n|⚜️ ايديه » '..member_id..' «')
...
return "|⚜️ ايدي البوت : "..msg.to.id.. "\n\n|⚜️ ايديك : "..msg.from.id.. "\n\n|⚜️ المطور : @IH_BI"
    ...
    elseif matches[1]:lower() == 'ايدي' then
      ...
      savelog(msg.to.id, name.." ["..msg.from.id.."] used /id ")
      return "|⚜️ ايدي المجموعه" ..string.gsub(msg.to.print_name, "_", " ").. ":\n\n"..msg.to.id
    end
  end
  ...
  if matches[1]:lower() == 'حظر' then-- /ban
    ...
    elseif string.match(matches[2], '^%d+$') then
        ...
        if not is_admin1(msg) and is_momod2(matches[2], msg.to.id) then
          	return "لا تستطيع حظر المشرفين😼👏🏻"
        end
        if tonumber(matches[2]) == tonumber(msg.from.id) then
          	return "لا تستطيع طرد نفسك غبي😼👏🏻"
        end
        local print_name = user_print_name(msg.from):gsub("‮", "")
	    local name = print_name:gsub("_", "")
		local receiver = get_receiver(msg)
		...
		send_large_msg(receiver, '|⚜️ العضو['..matches[2]..']  تم حظره')
      else
		local cbres_extra = {
		chat_id = msg.to.id,
		get_cmd = 'حظر',
      ...
      if string.match(targetuser, '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then
        	...
        	banall_user(targetuser)
       		return '|⚜️ العضو ['..user_id..' ] تم حظره عام'
     else
	local cbres_extra = {
		chat_id = msg.to.id,
		get_cmd = 'حظر عام',
    ...
    local chat_id = msg.to.id
      if string.match(matches[2], '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then
        	...
        	return '|⚜️ العضو'..user_id..' تم الغاء حظره عام'
    else
		local cbres_extra = {
			chat_id = msg.to.id,
			get_cmd = 'الغاء العام',
